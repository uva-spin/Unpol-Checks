...
class WorkflowTests(unittest.TestCase):
    ...
    def test_main_result_ensemble_rebuild(self):
        subprocess.run([sys.executable, "main-result/scripts/rebuild_main_bands.py", "--check"], cwd=ROOT, check=True)

    def test_finite_y(self):
        subprocess.run([sys.executable, "finite-y/scripts/verify_finite_y.py"], cwd=ROOT, check=True)

    def test_finite_y_numerical_refit(self):
        subprocess.run([sys.executable, "finite-y/scripts/reproduce_boundary_fit.py", "--check"], cwd=ROOT, check=True)
